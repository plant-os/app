...
class CropsBloc extends Bloc<CropsEvent, CropsState> {
  ...
  Stream<CropsState> mapEventToState(CropsEvent event) async* {
    ...
    // else if (event is ClickAddCrop) {
    //   yield* _mapAddCropToState(event);
    // }
    else if (event is ClickOnCrop) {
      yield* _mapClickCropToState(event);
    }
    // else if (event is ClickUpdateCrop) {
    //   yield* _mapUpdateCropToState(event);
    // } else if (event is ClickDeleteCrop) {
  ...
  // Stream<CropsState> _mapAddCropToState(ClickAddCrop event) async* {
  //   yield AddCropsState();
  // }

  // Stream<CropsState> _mapUpdateCropToState(ClickUpdateCrop event) {}

  // Stream<CropsState> _mapDeleteCropToState(ClickDeleteCrop event) {}

  Stream<CropsState> _mapClickCropToState(ClickOnCrop event) async* {
    yield CropDetailsState();
  }
}
