...
void main() {
  group(
    ...
    () {
      test(
          "Once redirected to the crops page loads all the crops from firebase",
          () {
        ...
        // when(authService.getCurrentUser()).thenAnswer((_) {
        //   return firebaseUser;
        // });

        CropsBloc cropsBloc = CropsBloc(authService, cropsService, userService);

        cropsBloc.add(CropsLoaded([crop, crop], user));

        // when(cropsBloc.cropsService.getCropslist("3S220zGg1GghXT075bcH"))
        //     .thenAnswer();
        expectLater(
          cropsBloc,
          emitsInOrder([CropsState(), CropsState()]),
        );
      });
    },
  );
